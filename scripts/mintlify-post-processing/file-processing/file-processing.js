...
const LINKED_TYPES_FILE = path.join(DOCS_DIR, '.linked-types.json');
...
function generateDocsJson(docsContent) {
  ...
  if (docsContent.functions.length > 0) {
    groups.push({
  ...
  if (docsContent.interfaces.length > 0) {
    groups.push({
  ...
  if (docsContent.classes.length > 0) {
    groups.push({
  ...
  if (docsContent.typeAliases.length > 0) {
    groups.push({
 ...
 */
function processAllFiles(dir, linkedTypeNames) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      processAllFiles(entryPath, linkedTypeNames);
      ...
      // Remove suppressed linked type files
      if (linkedTypeNames.has(fileName)) {
...
function main() {
  ...
  processAllFiles(DOCS_DIR, linkedTypeNames);
