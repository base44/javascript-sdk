...
describe("Core Integrations - InvokeLLM", () => {
  ...
  test("passes model parameter to the API", async () => {
    platform.given.integrations.llmResponds("Quantum computing uses qubits...");
    const params = { prompt: "Explain quantum computing", model: "gpt_5" };
    await expect(base44.integrations.Core.InvokeLLM(params)).resolves.toBe("Quantum computing uses qubits...");
  ...
  test("passes model alongside other optional parameters", async () => {
    ...
    platform.given.integrations.llmResponds(response);
    const params = {
      prompt: "Analyze this text",
      model: "claude_sonnet_4_6" as const,
      response_json_schema: { type: "object", properties: { sentiment: { type: "string" } } },
    };
    await expect(base44.integrations.Core.InvokeLLM(params)).resolves.toEqual(response);
    expect(platform.requests.last("integrations.invoke").body).toEqual(params);
  });
});
