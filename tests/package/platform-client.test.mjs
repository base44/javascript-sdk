...
test("the tarball contains both compiled entry points and no source or test trees", () => {
  ...
  for (const file of ["dist/index.js", "dist/index.d.ts", "dist/platform/client/index.js", "dist/platform/client/index.d.ts"]) assert.ok(files.has(file), file);
...
for (const [entry, forbidden, blockedEntry] of [
  ["@base44/sdk", "/dist/platform/", "@base44/sdk/platform/client"],
  ["@base44/sdk/platform/client", "/dist/(?!platform/).*", "@base44/sdk"],
]) {
...
void [runtime, createAxiosClient, explicit];
`;
